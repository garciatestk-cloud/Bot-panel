...
module.exports = async (client) => {
    ...
    try {
        const channelMM = await client.channels.fetch(config.MIDDLEMAN_PANEL_CHANNEL);
        if (channelMM) {
            const messages = await channelMM.messages.fetch({ limit: 10 });
            const oldPanelMM = messages.find(m => m.author.id === client.user.id);
            if (oldPanelMM) await oldPanelMM.delete().catch(() => {});
        ...
        console.error("Error cargando panel de Postulación Middleman:", error);
    }
